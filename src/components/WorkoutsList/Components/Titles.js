...
const Titles = ({status, f}) => {
  return (
    <ul>
      <li className={styles.titles}>
        <div className={styles.xsBlock + ' ' + styles.active} onClick={() => f('id', 0)}>
          {'id' + ((status[0].active ? ' \u25B4' : ' \u25BE'))}
        </div>
        <Other fill={'rgba(0,0,0,0)'}/>
        <div className={styles.sBlock + ' ' + styles.active} onClick={() => f('timestamp', 2)}>
        ...
        <div className={styles.blockMetricContainer}>
          <div className={styles.mBlock + ' ' + styles.active} onClick={() => f('totalDistance', 4)}>
            {dict.fields.totalDistance[userLang] + ((status[4].active ? ' \u25B4' : ' \u25BE') )}
          </div>
          <div className={styles.mBlock + ' ' + styles.active} onClick={() => f('totalTimerTime', 5)}>
            {dict.fields.time[userLang] + ((status[5].active ? ' \u25B4' : ' \u25BE') )}
          </div>
          <div className={styles.mBlock + ' ' + styles.active} onClick={() => f('enhancedAvgSpeed', 6)}>
            {dict.fields.avgSpeed[userLang] + ((status[6].active ? ' \u25B4' : ' \u25BE') )}
          </div>
          <div className={styles.mBlock + ' ' + styles.active} onClick={() => f('totalAscent', 7)}>
            {dict.fields.totalAscent[userLang] + ((status[7].active ? ' \u25B4' : ' \u25BE') )}
          </div>
          <div className={styles.mBlock + ' ' + styles.active} onClick={() => f('avgHeartRate', 8)}>
